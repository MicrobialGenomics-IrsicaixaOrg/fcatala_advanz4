...
dir.create(here::here("data", "source_data"))
...
```{r}
plt_list %>% 
  purrr::map_dfr(~ .x %>% purrr::pluck("lon_stats")) %>% 
  readr::write_tsv(
    here::here("data", "source_data", "fig_2B_lmm_longitudinal_stats.tsv")
  )

plt_list %>% 
  purrr::map_dfr(~ .x %>% purrr::pluck("grp_stats")) %>% 
  readr::write_tsv(
    here::here("data", "source_data", "fig_2B_lmm_group_stats.tsv")
  )
```
...
```{r}
plt_df %>% 
  readr::write_tsv(here::here("data", "source_data", "sup_fig_3B_rm_correlation.tsv"))
```
...
plt_df %>% 
  readr::write_tsv(
    here::here("data", "source_data", "fig_2C_rm_correlation_all.tsv")
  )
```
...
plt_list %>%
  ...
  readr::write_tsv(
    here::here("data", "source_data", "sup_fig_3C_rm_scat.tsv")
  )
```
   ...
   all_markers %>% 
      ...
      purrr::map(function(.marker) {
        ...
        plt_df <-
          it_data$df %>%
          purrr::list_rbind() %>%
          dplyr::rename(metric_value = 1, marker_value = 2) %>% 
          dplyr::mutate(metric_name = .metric, marker_name = .marker, .before = 1) %>% 
          dplyr::mutate(
            fitted_vals = c(
        ...
        plt <- 
          plt_df %>% 
          tidyplots::tidyplot(x = metric_value, y = marker_value, color = treatment) %>%
          tidyplots::add_data_points(alpha = 0.4) %>%
          tidyplots::add(geom_line(aes(y = fitted_vals, group = record_id), alpha = 0.8)) %>% 
          tidyplots::adjust_x_axis_title(
            .metric %>% stringr::str_replace_all("_", " ") %>% stringr::str_to_title()
...
plt_list %>%
  ...
  purrr::map_dfr(~ .x$data) %>% 
  readr::write_tsv(
    here::here("data", "source_data", "fig_2D_rm_scat.tsv")
  )
```
