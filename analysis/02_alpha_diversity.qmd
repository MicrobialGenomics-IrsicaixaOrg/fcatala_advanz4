...
> [!IMPORTANT]
> Because this was a randomized clinical trial, randomization ensures that baseline characteristics are, on average, balanced across treatment arms, minimizing the risk of confounding. Therefore, all analyses were performed using treatment assignment as the primary exposure variable. Adjustment for baseline covariates (age, sex, and risk group) was included solely to increase precision and account for inter-individual variability, not to control for confounding. Only baseline values were used for these covariates, as post-baseline measurements could be affected by treatment or follow-up time and would therefore not be appropriate for adjustment.
  ...
  purrr::map( ~ {
    ...
    lon_contrast_df <- 
      ...
      dplyr::mutate(
        ...
        time_point = readr::parse_number(contrast_clean) %>% as.character()
      )
    ...
    traj_plt <- 
      lon_contrast_df %>%
      dplyr::mutate(teatment = factor(treatment, levels = c("DRV/r", "DTG"))) %>%
      tidyplots::tidyplot(
        x = time_point, 
        y = estimate, 
        colour = treatment, 
      ...
      tidyplots::adjust_x_axis_title("Time Point (weeks)") %>%
      tidyplots::adjust_y_axis_title("Δ log2(outcome) vs baseline") %>%
      tidyplots::adjust_legend_title("Treatment")
...
> The trajectory plot shows the estimated mean change (Δ log₂) from baseline for each treatment group over time. A value of 0 indicates no change relative to baseline. Points represent model-adjusted estimates from the linear mixed model, and vertical bars indicate 95% confidence intervals. Differences between groups at a given visit are visualized by divergence of the trajectories.

> The forest plot displays the adjusted mean differences (Δ log₂) between treatments (DTG − DRV/r) at each visit. Positive estimates indicate higher values in the DTG arm, while negative estimates indicate higher values in the DRV/r arm. Error bars represent 95% confidence intervals; intervals crossing zero denote non-significant differences after FDR correction.
...
plt_df <- 
  sparman_corr_df %>% 
   dplyr::mutate(sign = dplyr::case_when(p_adj < 0.05  ~ "< 0.05", TRUE ~ "ns"))
...
plt_list <- 
  ...
  purrr::map(~ {
    ...
    plt <- 
      ...
      scale_fill_gradientn(
        colours = rev(RColorBrewer::brewer.pal(11, "PiYG")),
        limits = c(-lim, lim), breaks = seq(-lim, lim, by = 0.6), 
      ) +
      scale_colour_manual(values = c("< 0.05" = "black", "ns" = "white")) +
      scale_alpha_manual(values = c("< 0.05" = 0.9, "ns" = 0.6), guide = "none") +
      scale_size_continuous(range = c(2, 8), guide = "none") +
...
### Compute Repeated Measures Correlation Coefficient (plt 2)
...
plt_df <-
  rmcorr_df %>%
  dplyr::filter(metric != "pielou_evenness") %>% 
  dplyr::mutate(
    marker = dplyr::case_when(
      marker == "CD4" ~ "CD4+ T cells", 
      marker == "CD8" ~ "CD8+ T cells", 
      marker == "CD4_CD38_DR" ~ "CD4+ CD38+\n HLA-DR+ T cells", 
      marker == "CD8_CD38_DR" ~ "CD8+ CD38+\n HLA-DR+ T cells", 
      marker == "IL6" ~ "IL-6", 
      marker == "TNFa" ~ "TNF-a", 
      TRUE ~ marker
    ),
    ...
    marker = factor(marker, levels = all_markers_2), 
    metric = stringr::str_replace_all(metric, "_", " ") %>% stringr::str_to_title(),
    metric = factor(metric, levels = c("Shannon Diversity", "Observed Richness", "Gini Dominance", "Gene Richness"))
  )

# plot
plt <-
  plt_df %>%
  ggplot(aes(x = treatment, y = marker, fill = cor, colour = sign)) +
  geom_point(aes(size = abs(cor), alpha = sign), shape = 22, stroke = 1) +
