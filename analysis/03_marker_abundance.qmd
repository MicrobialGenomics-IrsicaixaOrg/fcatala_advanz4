...
cytokines <- c("CRP", "IL-6", "TNF-α", "sCD14")
...
pc <- min(unlist(min_df), na.rm = TRUE) %>%
  {./10} %>%
  {./2}
```


## Plot Global Evolution - Log2(x + pc) Transformation
...
plt_list <- 
  ...
  purrr::map(~ {
    ...
    plt_df <- 
      ...
      dplyr::mutate(time_point = factor(time_point)) %>%
      dplyr::mutate(!!dplyr::sym(.x) := log2(!!dplyr::sym(.x)+pc))
    ...
    plt <- 
      ...
      tidyplots::add_test_asterisks(
        method = "wilcox_test", hide_info = TRUE, bracket.nudge.y = 0.05, tip.length = 0.01) %>%
      tidyplots::add(ggpubr::stat_pvalue_manual(
        stats_2, label = "p.adj.signif", hide.ns = TRUE, tip.length = 0.01, step.increase = 0.15
      )) %>% 
      tidyplots::adjust_x_axis_title("Time Point (weeks)") %>%
      tidyplots::adjust_legend_title("Treatment") %>%
      tidyplots::adjust_y_axis_title(
        glue::glue("{.x} ({.unit}, log2(x + {pc}))")
      ) %>% 
      tidyplots::add(theme(
    ...
    # save
    plt %>%
      tidyplots::adjust_size(width = 40, height = 45, unit = "mm") %>%
      tidyplots::save_plot(
        here::here(out_dir, glue::glue("temp_abundance_{.x}.pdf")),
        view_plot = FALSE
      )
