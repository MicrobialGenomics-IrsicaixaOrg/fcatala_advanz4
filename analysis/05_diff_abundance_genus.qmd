...
res_df <- 
  ...
  dplyr::select(taxon, Genus, Genus, prevalence) %>% 
...
res_df %>%
  ...
  dplyr::mutate(taxon = forcats::fct_reorder(taxon, lfc)) %>%
  tidyplots::tidyplot(x = lfc, y = taxon) %>%
  ...
  tidyplots::adjust_x_axis(limits = c(-8, 8)) %>% 
...
bar_plts <- 
  ...
  purrr::map2(tp_palette, ~ {
    bar_plt <- 
      ...
      tidyplot(abs_lfc, Genus, color = sign) %>%
...
evolution_plt <- 
  plot_df_2 %>% 
  ggplot(aes(time_point, lfc, colour = Genus, group = Genus)) +
...
plt <-
  plt_df %>%
  ggplot(aes(x = Genus, y = marker, fill = cor, colour = sign)) +
  geom_point(aes(size = abs(cor), alpha = sign), shape = 22, stroke = 1) +
  scale_fill_gradientn(
    colours = rev(RColorBrewer::brewer.pal(11, "RdYlGn")),
    limits = c(-lim, lim)
