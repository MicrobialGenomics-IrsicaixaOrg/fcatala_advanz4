...
plot_df <- 
  res_df %>% 
  dplyr::mutate(
    candidate = abs(lfc) >= 0.5 & qval < 0.01 & passed & prevalence > 0.2,
...
### Volcano by Time-Point

```{r}
levels(plot_df$time_point) %>%
  purrr::set_names() %>%
  purrr::map2(seq_along(.), ~ {
    ...
    volc_plt <-
      ...
      adjust_colors(colors_discrete_friendly[.y]) %>% 
      adjust_x_axis_title("$Log[2]~fold~change$") %>%
      adjust_y_axis_title("$-Log[10]~italic(P)~adjusted$") %>%
      adjust_legend_title("Time Point") %>%
...
bar_plt <- 
  ...
  dplyr::mutate(
    ...
    Genus = stringr::str_c(Genus, "_", time_point, "w"), 
...
lolli_plt <- 
  ...
  tidyplot(abs_lfc, Genus, color = sign, size = prevalence) %>% 
...
evolution_plt <- 
  ...
  coord_cartesian(clip = "off", ylim = c(-1, 2.1)) +
