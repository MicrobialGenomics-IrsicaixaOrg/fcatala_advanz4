---
title: "05 - Beta Diversity Analysis"
---
...
# output
out_dir <- here::here("data", "05_beta_diversity")
...
### Scatter Plot by Timepoint (NMDS1 vs NMDS2)
...
plt_list <- 
  ...
  purrr::map( ~ {
    ...
    plot_df <- 
      tse %>% 
      dplyr::select(treatment, time_point, dplyr::contains("NMD")) %>% 
    ...
    p_box_x <-
      plot_df %>%
      ggplot(aes(x = time_point, y = NMDS1, colour = treatment, fill = treatment)) +
    ...
    p_box_y <- 
      plot_df %>% 
      ggplot(aes(x = time_point, y = NMDS2, colour = treatment, fill = treatment)) +
      geom_boxplot(alpha = 0.2, outlier.shape = NA) +
      geom_point(size = 0.8, alpha = 0.2, position = position_jitterdodge()) +
      scale_colour_manual(values = c("#0072B2", "#D55E00")) +
      scale_fill_manual(values = c("#0072B2", "#D55E00")) +
    ...
    ggsave(
      combined_plot, 
      filename =  here::here(out_dir, glue::glue("nmds_scatter_{.x}.pdf")),
      width = width_mm, height = height_mm, units = "mm"
    )
...
```{r}
plt_list %>% 
  purrr::map_dfr(~ .x[1]$data) %>% 
  readr::write_tsv(here::here("data", "source_data", "fig_3A_nmds.tsv"))
```
...
tse <- 
  ...
  dplyr::mutate(
    n_grp = dplyr::n(),
    mean1_loo = dplyr::if_else(
      n_grp > 1, 
      (sum(NMDS1, na.rm = TRUE) - NMDS1) / (n_grp - 1), 
      NMDS1
    ), 
    mean2_loo = dplyr::if_else(
      n_grp > 1, 
     (sum(NMDS2, na.rm = TRUE) - NMDS2) / (n_grp - 1),
      NMDS2   
    ),
    centroid_dist = sqrt((NMDS1 - mean1_loo)^2 + (NMDS2 - mean2_loo)^2)
  ) %>% 
  dplyr::ungroup() %>% 
...
plt_list <- 
  ...
  purrr::map( ~ {
    ...
    readr::write_csv(lon_contrast_df, here::here(out_dir, "lmm_longitudinal_stats.csv"))
    ...
    readr::write_csv(group_contrast_df, here::here(out_dir, "lmm_group_stats.csv"))
...
```{r}
plt_list$centroid_dist[[1]]$data %>% 
  readr::write_tsv(here::here("data", "source_data", "fig_3C_centorid_dist_longitudinal.tsv"))

plt_list$centroid_dist[[2]]$data %>% 
  readr::write_tsv(here::here("data", "source_data", "fig_3C_centorid_dist_group.tsv"))
```
...
tse2 <- 
  tse %>% 
  dplyr::mutate(dplyr::across(dplyr::all_of(all_markers_abb), ~ log2(.x + 1e-6)))
```
...
spearman_corr_df <-
  ...
  dplyr::mutate(
    ...
    Var2 = factor(Var2, levels = all_markers_long)
   )
...
plt_df <- 
  spearman_corr_df %>%
   dplyr::mutate(
    sign = dplyr::case_when(p_adj < 0.05 ~ "< 0.05", TRUE ~ "ns")
  )
...
rmcorr_df_all <- 
  tidyr::expand_grid(metric = "centroid_dist", all_markers_long) %>%
  purrr::pmap_dfr( ~ {
    # prepare data
    print(.y)
    f_df <-
...
plt_df %>%
  dplyr::mutate(lab = stringr::str_replace(metric, "_", " ") %>% stringr::str_to_sentence()) %>% 
  readr::write_tsv(here::here("data", "source_data", "sup_fig_4B_rmcorr.tsv"))
```
...
readr::write_csv(rmcorr_df, here::here(out_dir, "rmcorr_stats.csv"))
...
```{r}
plt_df %>%
  readr::write_tsv(here::here("data", "source_data", "fig_3F_rmcorr_treatment.tsv"))
```
...
names(plt_list) %>% 
  purrr::map_dfr(~ {
    plt_list[[.x]]$data %>% 
      dplyr::rename(marker_value = !!dplyr::sym(.x)) %>% 
      dplyr::mutate(marker_name = .x, .before = marker_value)
  }) %>% 
    readr::write_tsv(here::here("data", "source_data", "sup_fig_4C_rmcorr.tsv"))
```
...
names(plt_list) %>% 
  purrr::map_dfr(~ {
    ...
    readr::write_tsv(here::here("data", "source_data", "fig_3G_rmcorr_treatment.tsv"))
```
...
```{r}
permanova_tbl$`_data` %>% 
  readr::write_tsv(here::here("data", "source_data", "fig_3B_permanova.tsv"))
```
...
sample_order <- 
  ...
  get_sample_ordination(method = "NMDS", order_var = "MDS1", dist = "bray")
```
...
p2 <- 
  ...
  purrr::imap(~ {
    ...
    if (.x == "centroid_dist") {
      ...
      p <- p + scale_fill_manual(values = as.vector(col))
    }
    p
  }) %>% wrap_plots(ncol = 1)
```
...
```{r}
p1$data %>% 
  readr::write_tsv(here::here("data", "source_data", "sup_fig_4A_gene_richness.tsv"))

p3$data %>% 
  readr::write_tsv(here::here("data", "source_data", "sup_fig_4A_rel_abundance.tsv"))
```
