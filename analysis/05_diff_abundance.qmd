...
res_df %>%
  ...
  dplyr::slice_min(qval, n = 40) %>% 
  # dplyr::ungroup() %>% 
  dplyr::mutate(Species = forcats::fct_reorder(Species, lfc)) %>%
  tidyplots::tidyplot(x = lfc, y = Species) %>%
  ...
  # tidyplots::adjust_x_axis(limits = c(-3, 3)) %>% 
  tidyplots::adjust_font(fontsize = 15) %>% 
  tidyplots::adjust_size(100, 100) %>%
  tidyplots::split_plot(time_point, guides = "collect")
```
...
r_plt <-
  plt + plot_spacer() +
  plot_layout(
    widths = ggplot2::unit(c(250, 1), "mm"),
