...
all_markers_abb <- c("CD4", "CD8", "CD4_nadir", "CD4_CD38_DR", "CD8_CD38_DR", "CRP", "IL6", "TNFa", "sCD14", "HIV_VL", "BMI")

all_markers_long <- c("CD4+ T cells", "CD8+ T cells", "CD4+ T cell count nadir", "CD4+ CD38+ HLA-DR+ T cells", "CD8+ CD38+ HLA-DR+ T cells", "CRP", "IL-6", "TNF-α", "sCD14", "HIV viral load", "BMI")
```
...
# build the plot
save_timepoint_plot <- function(tp_value, df, out_dir, x_limits, y_limits) {
  df_tp <- df %>% dplyr::filter(time_point == tp_value)

  # main
  p_scatter <-
    df_tp %>%
    ggplot(aes(NMDS1, NMDS2)) +
    geom_point(aes(colour = treatment), alpha = 0.7) +
    stat_ellipse(aes(colour = treatment), alpha = 0.8) +
    scale_colour_manual(values = c("#0072B2", "#D55E00")) +
    coord_cartesian(xlim = x_limits, ylim = y_limits) +
    theme_classic() +
    theme(legend.position = "bottom") +
    labs(title = paste("Timepoint:", tp_value), 
         x = "NMDS1", 
         y = "NMDS2",
         colour = "Treatment")
  ...
  p_box_x <-
    ...
    geom_boxplot(alpha = 0.8, outlier.shape = NA) +
    geom_jitter(width = 0.2, height = 0, size = 1.5, alpha = 0.4) +
    scale_colour_manual(values = c("#0072B2", "#D55E00")) +
    ggpubr::stat_compare_means(method = "wilcox.test",
                               label = "p.signif", label.x = 2, size = 4) +
    coord_flip(ylim = x_limits) +
    theme_void() +
    theme(legend.position  = "none") +
    ggtitle("NMDS1 distribution")
  ...
  p_box_y <-
    ...
    scale_colour_manual(values = c("#0072B2", "#D55E00")) +
    ggpubr::stat_compare_means(method = "wilcox.test",
                               label = "p.signif", label.x = 1.75, size = 4) +
    coord_cartesian(ylim = y_limits) +
    theme_void() +
    theme(legend.position = "none") +
    ggtitle("NMDS2 distribution")

  # assemble
  combined <-
    p_box_x + patchwork::plot_spacer() + p_scatter + p_box_y +
    patchwork::plot_layout(
      widths  = unit(c(60, 10), "mm"),
      heights = unit(c(10, 60), "mm"),
      ncol = 2
    )
  # compute panel-specific size
  grob <- patchworkGrob(combined)
  width_mm  <- grid::convertWidth(sum(grob$widths),  "mm", valueOnly = TRUE) * 1.5
  height_mm <- grid::convertHeight(sum(grob$heights), "mm", valueOnly = TRUE)

  # save PDF
  out_pdf <- here::here(out_dir, paste0("nmds_scatter_timepoint_", tp_value, ".pdf"))
  ggsave(plot = combined, filename = out_pdf, width = width_mm, height = height_mm, units = "mm")

}

unique(plot_df$time_point) %>%
  purrr::walk(~ save_timepoint_plot(tp_value = .x,
                             df = plot_df,
                             out_dir = out_dir,
                             x_limits = x_limits,
                             y_limits = y_limits))
...
```{r}
# Log2 Transformation
tse2 <- tse %>% dplyr::mutate(dplyr::across(dplyr::all_of(all_markers_abb), ~ log2(.x + 1e-6)))
```
