...
plt <-
  plt_df %>%
  dplyr::mutate(
    lab = stringr::str_replace(metric, "_", " ") %>% stringr::str_to_sentence()
  ) %>%
  ...
  scale_colour_manual(
    values = c("< 0.05" = "black", "< 0.1" = "#525252", "ns" = "white")
  ) +
  scale_alpha_manual(
    values = c("< 0.05" = 0.9, "< 0.1" = 0.9, "ns" = 0.6),
    guide = "none"
  ) +
  scale_size_continuous(range = c(2, 8), guide = "none") +
  theme_minimal() +
    ...
    it_data <- 
      ...
      dplyr::mutate(
        marker = dplyr::case_when(
          ...
          marker == "CD4_nadir" ~ "CD4+ T cell count nadir",
          marker == "CD4_CD38_DR" ~ "CD4+ CD38+ HLA-DR+ T cells",
          marker == "CD8_CD38_DR" ~ "CD8+ CD38+ HLA-DR+ T cells",
          marker == "IL6" ~ "IL-6",
          marker == "CRP" ~ "CRP",
          marker == "TNFa" ~ "TNF-α",
          marker == "HIV_VL" ~ "HIV viral load", 
          TRUE ~ marker
        ), 
        marker = factor(marker, levels = all_markers_long)
      ) %>% 
      dplyr::filter(metric == .metric, marker == .marker)
